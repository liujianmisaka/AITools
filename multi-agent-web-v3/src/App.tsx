...
function JobComposer({ submitting, error, onClose, onSubmit }: { submitting: boolean; error?: string; onClose: () => void; onSubmit: (payload: JobSubmission) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="modal" onSubmit={submit}><div className="modal-header"><div><span className="eyebrow">NEW INVOCATION</span><h2>创建任务</h2></div><button type="button" className="icon-button" onClick={onClose}><X size={18} /></button></div><label>任务 ID<input value={jobId} onChange={(event) => setJobId(event.target.value)} required /></label><div className="form-grid"><label>模型<input value={model} onChange={(event) => setModel(event.target.value)} required /></label><label>推理等级<select value={effort} onChange={(event) => setEffort(event.target.value)}><option value="low">low</option><option value="medium">medium</option><option value="high">high</option><option value="xhigh">xhigh</option></select></label></div><label>任务内容<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="描述希望 Agent 执行的内容…" rows={6} required /></label>{error && <div className="error-banner">{error}</div>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" disabled={submitting}>{submitting ? <LoaderCircle className="spin" size={16} /> : <Activity size={16} />}运行任务</button></div></form></div>
}
