...
def _provider_id(config: JsonObject) -> str:
    ...
    if provider_id not in {"openai", "ollama", "lmstudio", "amazon-bedrock"}:
        ...
        if not isinstance(providers, dict) or not isinstance(providers.get(provider_id), dict):
            raise ValueError(
                f"Codex model_provider {provider_id!r} has no matching provider definition"
            )
    return provider_id
...
def _environment_kind(
    ...
    if provider_id == "openai" and catalog_path is None and not config.get("openai_base_url"):
...
class CodexRuntimeLocator:
    ...
    def _home(self) -> tuple[Path, str]:
        if self._codex_home is not None:
            return _expanded_path(str(self._codex_home), self._user_home).resolve(), "explicit"
        environment_home = self._environ.get("CODEX_HOME")
        if environment_home and environment_home.strip():
            return _expanded_path(environment_home, self._user_home).resolve(), "environment"
    ...
    def resolve(self) -> CodexRuntimeDescriptor:
        ...
        identity = f"{self._codex_bin or 'sdk'}\0{codex_home}".encode()
        runtime_id = (
            f"codex:{environment_kind.value}:{provider_id}:"
